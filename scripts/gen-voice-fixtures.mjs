...
function run(args, dest) {
  ...
  execFileSync("ffmpeg", ["-hide_banner", "-loglevel", "error", "-y", ...args, tmp], {
    stdio: "inherit",
  });
...
function main() {
  ...
  run(
    [
      ...
      "lavfi",
      "-i",
      "sine=frequency=50:duration=4",
      "-f",
      "lavfi",
      "-i",
      "sine=frequency=100:duration=4",
      "-f",
      "lavfi",
      "-i",
      "sine=frequency=150:duration=4",
      "-filter_complex",
      rumble,
      "-map",
      "[out]",
  ...
  console.log(
    `gen-voice-fixtures: wrote ${out("clean.wav")}, ${out("rumble.wav")}, ${out("silence.wav")}`,
  );
}
