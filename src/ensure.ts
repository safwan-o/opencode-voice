...
export async function ensureDownloaded(
  env: EnsureEnv,
  model = getModel(env.settings.model),
): Promise<boolean> {
  ...
  await download(model, env.options, env.settings, {
    ...
    onRetry: ({
      ...
      env.toast(
        `Download retry ${nextAttempt}/${attempts}: ${error instanceof Error ? error.message : String(error)}`,
        "warning",
      ),
  });
...
export async function ensureEngineReady(
  ...
  await install(engineId, commandOptions, env.settings, {
    ...
    onRetry: ({
      ...
      env.toast(
        `Engine install retry ${nextAttempt}/${attempts}: ${error instanceof Error ? error.message : String(error)}`,
        "warning",
      ),
  });
...
export async function ensureRecorderReady(env: EnsureEnv): Promise<boolean> {
  ...
  const commandOptions = {
    ...env.options,
    downloadDir: env.settings.downloadDir,
  ...
  await ensure(commandOptions, env.settings, {
    ...
    onRetry: ({
      ...
      env.toast(
        `Recorder install retry ${nextAttempt}/${attempts}: ${error instanceof Error ? error.message : String(error)}`,
        "warning",
      ),
  });
